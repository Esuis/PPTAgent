...
# 新增：解决Docker API版本过低问题 - 全局设置兼容的API版本（核心修复）
...
ENV PATH="/opt/.venv/bin:${PATH}" \
    ...
    # 追加：将DOCKER_API_VERSION加入环境变量（确保全局生效）
    DOCKER_API_VERSION=1.44

# Export ENV to /etc/profile.d/ for bash -lc and interactive shells
RUN printenv | grep -E '^(PATH|PYTHONUNBUFFERED|VIRTUAL_ENV|PUPPETEER_|LANG|LC_ALL|MPLCONFIGDIR|MCP_CLIENT_DOCKER|DOCKER_API_VERSION)=' | sed 's/^/export /' > /etc/profile.d/docker-env.sh && \
